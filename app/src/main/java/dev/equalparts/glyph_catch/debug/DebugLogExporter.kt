        ...
        file.bufferedWriter().use { writer ->
            for (event in events) {
                ...
                val line = buildJsonObject {
                    ...
                    put("phoneBattery", JsonPrimitive(event.phoneBattery))
                    put("phoneIsInteractive", JsonPrimitive(event.phoneIsInteractive))
                    put("phoneMinutesOff", JsonPrimitive(event.phoneMinutesOff))
                    put("phoneMinutesOffOutsideBedtime", JsonPrimitive(event.phoneMinutesOffOutsideBedtime))
                    put("queueSize", JsonPrimitive(event.queueSize))
                    put("hasSleepBonus", JsonPrimitive(event.hasSleepBonus))
                    put("isBedtime", JsonPrimitive(event.isBedtime))
